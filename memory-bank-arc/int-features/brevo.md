...
| Path | Purpose |
|---|---|
| `frontend-tenant/src/app/settings/integrations/page.tsx` | Enhanced `BrevoIntegrationCard` with setup wizard, guide, toasts, **Send Test Email** dialog, and **Connected (Master Key)** badge when only the platform master key is configured |

### 11.4 Phase-6 (2026-07-25) — Master-key UX + Send Test Email

> Phase-6 closes D-08 (integrations testability) and D-09 (master-key UX).
> The tenant UI now exposes a "Send Test Email" dialog that calls
> `POST /api/v1/integrations/brevo/test-send` and reports the live
> `messageId` from Brevo's SMTP relay.

| File | Change |
|------|--------|
| `backend/src/modules/integrations/integrations.service.ts:336-348` | `getBrevoConnectionStatus` returns `{ connected, source: 'tenant' \| 'master' }` |
| `frontend-tenant/src/app/settings/integrations/page.tsx` | New "Send Test Email" dialog + master-key badge |
| `frontend-tenant/src/services/integrations.service.ts:175` | New `brevoTestSend(payload)` method |

**Live evidence (2026-07-25):** `POST /api/v1/integrations/brevo/test-send` with the demo tenant's cookies returned `messageId=<202607250454.93699028513@smtp-relay.mailin.fr>` (delivered via Brevo's outbound relay). The `source: 'master'` field confirms the platform fallback key was used.

See [audits/2026-07-25-industry-verification-3-remediation/REPORT.md](../audits/2026-07-25-industry-verification-3-remediation/REPORT.md) §3.3.
