...
import type {
  IDashboardService,
  DailyBriefing,
} from "@/core/services/interfaces/IDashboardService";
...
export class DashboardService implements IDashboardService {
  ...
  async getCompanyMetrics(
    _timeRange?: import("@/shared/types/domain.types").TimeRange,
    ...
    const activeAgents = agentResult.items.filter(
      (a) => a.status === "RUNNING" || a.status === "IDLE",
    ).length;
    const pendingTasks = taskResult.items.filter(
      (t) => t.status === "PENDING",
    ).length;
    const inProgressTasks = taskResult.items.filter(
      (t) => t.status === "IN_PROGRESS",
    ).length;
    const completedTasks = taskResult.items.filter(
  ...
  async getDailyBriefing(): Promise<DailyBriefing> {
    ...
    const criticalTasks = taskResult.items.filter(
    ...
    return {
      headline: `${agentResult.items.filter((a) => a.status === "RUNNING" || a.status === "IDLE").length} of ${agentResult.total} agents deployed`,
  ...
  async getPendingWorkItems(): Promise<{
    tasks: Task[];
    workflows: Workflow[];
    ...
    const [taskResult, workflowResult] = await Promise.all([
      ...
      this.workflows.findAll({ status: "ACTIVE" } as QP),
    ]);
  ...
  private _calculateTeamHarmony(agents: Agent[]): number {
    ...
    const avgSuccess =
      active.reduce((s, a) => s + (a.performance?.successRate ?? 0), 0) /
      active.length;
    ...
    return Math.max(
      0,
      Math.min(100, Math.round(avgSuccess - stressPenalty - errorPenalty)),
    );
  }
}
