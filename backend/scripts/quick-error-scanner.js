...
function scanDirectory(dirPath, component) {
  ...
  console.log(
    `  Sampling: ${sampleFiles.length} files for quick diagnostics\n`,
  );
  ...
  sampleFiles.forEach((file) => {
    try {
      ...
      if (
        content.includes('unknown type') ||
        content.includes('Type not found')
      ) {
        sampleErrors++;
      }
    } catch (e) {
...
async function main() {
  ...
  if (issues.import_errors.length > 0) {
    console.log(
      `${colors.red}Import Errors Found: ${issues.import_errors.length}${colors.reset}`,
    );
    ...
    console.log(
      `${colors.green}✓ No obvious import errors found${colors.reset}\n`,
    );
  }

  console.log(
    `${colors.yellow}Note: This is a quick scan. Full TypeScript compilation may reveal additional issues.${colors.reset}`,
  );
  console.log(
    `${colors.blue}═══════════════════════════════════════════════════${colors.reset}\n`,
  );
