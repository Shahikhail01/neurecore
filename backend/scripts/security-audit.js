...
class SecurityAudit {
  ...
  auditTenantIsolation() {
    ...
    if (fs.existsSync(authPath)) {
      ...
      const hasTenantCheck =
        content.includes('tenantId') && content.includes('CurrentTenant');
    ...
    const dbPath = path.join(
      this.workspaceRoot,
      'backend/src/modules/nocobase/database',
    );
    ...
    if (fs.existsSync(adminRoutesPath)) {
      const hasAuthGuard = this.checkFileContent(
        adminRoutesPath,
        /useAuth|ProtectedRoute|withAuth/,
        10,
      );
      ...
      this.log(
        `  ${hasAuthGuard > 0 ? '✅' : '⚠️'} Frontend auth guards (${hasAuthGuard} found)`,
      );
    }
    ...
    this.log(
      `  ${tenantValidationCount > 0 ? '✅' : '⚠️'} Tenant validation in ${tenantValidationCount} controllers`,
    );
    ...
    checks.forEach((check) => {
      ...
      if (check.passed) {
        ...
        this.results.audit.tenantIsolation[
          check.severity === 'PASS' ? 'passed' : 'warnings'
        ]++;
      }
    });
  }
  ...
  auditJwtValidation() {
    ...
    const hasExpiry =
      (hasJwtGuard && fs.readFileSync(jwtGuardPath, 'utf8').includes('exp')) ||
    ...
    const secretsInCode = this.checkFileContent(
      backendPath,
      /secret|password|key.*=.*['\"][a-zA-Z0-9]{8,}['\"]/,
      50,
    );
    ...
    this.log(
      `  ${secretsInCode === 0 ? '✅' : '⚠️'} Hardcoded secrets (${secretsInCode} potential matches)`,
    );
  ...
  auditPermissionMatrix() {
    ...
    const roleEnumPath = path.join(
      this.workspaceRoot,
      'backend/src/modules/auth/enums/user-role.enum.ts',
    );
    ...
    const aclPath = path.join(
      this.workspaceRoot,
      'backend/src/modules/auth/decorators/require-role.decorator.ts',
    );
    const hasPermDecorators =
      fs.existsSync(aclPath) ||
    ...
    const aclModulePath = path.join(
      this.workspaceRoot,
      'backend/src/modules/nocobase/acl',
    );
    ...
    this.log(
      `  ${hasAclModule ? '✅' : '⚠️'} ACL module (${hasAclModule ? 'integrated' : 'not found'})`,
    );

    // Check 4: API endpoints require authentication
    const apiPath = path.join(this.workspaceRoot, 'backend/src/modules');
    const protectedEndpoints = this.findFiles(
      apiPath,
      /\.controller\.ts$/,
      30,
    ).filter((file) => {
    ...
    this.log(
      `  ${protectedEndpoints > 0 ? '✅' : '⚠️'} Protected endpoints (${protectedEndpoints} with guards)`,
    );
  ...
  auditDatabaseFiltering() {
    ...
    if (hasPrismaSchema) {
      ...
      const hasTenantField =
        schema.includes('tenantId') || schema.includes('tenant_id');
    ...
    const interceptorsPath = path.join(
      this.workspaceRoot,
      'backend/src/modules/auth/interceptors',
    );
    const hasFilterInterceptor =
      fs.existsSync(interceptorsPath) &&
      fs
        .readdirSync(interceptorsPath)
        .some((f) => f.includes('filter') || f.includes('tenant'));
    ...
    this.log(
      `  ${hasFilterInterceptor ? '✅' : '⚠️'} Query filter interceptors`,
    );
    ...
    const hasSafeQueries =
      this.checkFileContent(servicePath, /where.*tenant|filter.*tenant/, 20) >
      0;
  ...
  auditRouteProtection() {
    ...
    const hasProtectedRoutes =
      hasMiddleware && fs.readFileSync(middlewarePath, 'utf8').includes('auth');
    ...
    this.log(
      `  ${hasProtectedRoutes ? '✅' : '⚠️'} Route authentication checks`,
    );
  ...
  auditSensitiveData() {
    ...
    const envGitignored =
      fs.existsSync(path.join(this.workspaceRoot, '.gitignore')) &&
      fs
        .readFileSync(path.join(this.workspaceRoot, '.gitignore'), 'utf8')
        .includes('.env');
    ...
    const backendPath = path.join(this.workspaceRoot, 'backend/src');
    const apiKeyMatches = this.checkFileContent(
      backendPath,
    ...
    this.log(
      `  ${apiKeyMatches === 0 ? '✅' : '⚠️'} No hardcoded API keys (${apiKeyMatches} potential matches)`,
    );

    // Check 3: Secrets use environment variables
    const usesEnvVars =
      this.checkFileContent(backendPath, /process\.env\.[A-Z_]+/, 100) > 0;
      ...
      try {
        ...
        items.forEach((item) => {
          ...
          if (
            ...
            stat.isFile() &&
            ['.ts', '.js', '.tsx', '.jsx'].includes(path.extname(item))
          ) {
            try {
  ...
  generateReport() {
    ...
    Object.entries(auditSections).forEach(([name, section]) => {
      if (typeof section === 'object' && 'passed' in section) {
        ...
        const passRate =
          total > 0 ? ((section.passed / total) * 100).toFixed(0) : 'N/A';

        const status =
          section.failed === 0 ? '✅' : section.warnings > 0 ? '⚠️' : '❌';
        this.log(
          `${status} ${name.replace(/([A-Z])/g, ' $1').trim()}: ${section.passed}/${total} (${passRate}%)`,
        );
      }
    });
    ...
    this.log(`\n${'='.repeat(70)}`);
    this.log(
      `📊 OVERALL SECURITY SCORE: ${finalPassRate}%`,
      finalPassRate >= 80 ? 'green' : finalPassRate >= 60 ? 'yellow' : 'red',
    );
    this.log(
      `Total Checks: ${grandTotal} | ✅ ${totalPassed} | ❌ ${totalFailed} | ⚠️ ${totalWarnings}`,
    );
    this.log(`${'-'.repeat(70)}\n`);
    ...
    if (totalFailed === 0 && totalWarnings < 5) {
      ...
      this.log(
        'All critical security checks passed. System is secure.',
        'green',
      );
    ...
    const reportPath = path.join(
      this.workspaceRoot,
      'PHASE_12_SECURITY_AUDIT_REPORT.json',
    );
