...
export function TaskInspector({ id }: { id: string }) {
  ...
  useEffect(() => {
    api
      .get(`/tasks/${id}`, {
        params: isPlatformViewer ? { scope: "platform" } : undefined,
      })
  ...
  if (!task)
    return <div className="p-6 text-zinc-500 text-sm">Task not found.</div>;
    ...
    >
      <div>
        ...
        <div className="flex items-center gap-2 mt-1 flex-wrap">
          ...
          {task.agent && (
            <span className="text-xs text-zinc-600">👤 {task.agent.name}</span>
          )}
        </div>
      </div>
      ...
      {task.evaluationScore !== undefined && (
        <div className="flex justify-between items-center">
          ...
          <span
            className={`text-sm font-bold ${task.evaluationScore >= 0.8 ? "text-status-profit" : task.evaluationScore >= 0.5 ? "text-status-warn" : "text-status-risk"}`}
          >
      ...
      {task.cost !== undefined && (
        <div className="flex justify-between items-center">
          ...
          <span className="text-sm font-bold text-status-strategy">
            ${task.cost.toFixed(4)}
          </span>
        </div>
      )}
              ...
              <div key={i} className="flex items-start gap-3 text-xs">
                <span
                  className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-bold ${
                    step.status === "success"
                      ? "bg-status-profit/20 text-status-profit"
                      : step.status === "failed"
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {step.durationMs && (
                    <p className="text-zinc-600">{step.durationMs}ms</p>
                  )}
                  {step.output && (
