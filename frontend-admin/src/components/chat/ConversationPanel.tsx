'use client';
// ─── ConversationPanel (Admin Portal) ────────────────────────────────────────
// Mirror of tenant ConversationPanel — admin-scoped slash commands + indigo color
...
function renderMarkdown(text: string): string {
  return text
    ...
    .replace(/\*(.*?)\*/g, '<em>$1</em>')
    .replace(/`(.*?)`/g, '<code class="bg-surface-overlay px-1 rounded text-xs text-indigo-300">$1</code>')
    .replace(/\n/g, '<br/>');
}
    ...
    >
      ...
      >
        ...
        {msg.tokens && (msg.tokens.input > 0 || msg.tokens.output > 0) && (
          <div className="mt-1 text-[9px] text-zinc-600">{msg.tokens.input}↑ {msg.tokens.output}↓ tokens</div>
        )}
      </div>
    </motion.div>
  );
...
function InputBox({ onSend, disabled }: { onSend: (q: string, ctx?: ChatRequest['context']) => void; disabled: boolean }) {
  ...
  const handleChange = async (v: string) => {
    ...
    if (v.startsWith('/')) {
      ...
      if (slash) { setContext(slash.context); setSuggestions(slash.suggestions); return; }
    }
    setContext(undefined);
    if (v.length > 2) setSuggestions(await chatService.getSuggestions(v));
    else setSuggestions([]);
  };
  ...
  const submit = () => {
    ...
    if (!q) return;
    onSend(q, context);
    setValue(''); setSuggestions([]); setContext(undefined);
  };
        ...
        {suggestions.length > 0 && value.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 4 }}
            ...
            {suggestions.slice(0, 5).map((s) => (
              <button key={s} onClick={() => { setValue(s); setSuggestions([]); }}
                className="w-full text-left px-3 py-2 text-xs text-zinc-300 hover:bg-surface-overlay transition first:rounded-t-xl last:rounded-b-xl">{s}</button>
            ))}
          </motion.div>
        )}
      </AnimatePresence>
      <div className="flex gap-2 items-end border-t border-surface-border p-3">
        <textarea rows={1} value={value} onChange={(e) => void handleChange(e.target.value)} onKeyDown={onKey}
          disabled={disabled} placeholder="Ask anything… or type / for commands"
          className="flex-1 resize-none bg-surface-overlay rounded-lg px-3 py-2 text-xs text-zinc-200 placeholder-zinc-600 border border-surface-border focus:outline-none focus:border-indigo-600 transition min-h-[32px] max-h-[80px]" />
        <button onClick={submit} disabled={disabled || !value.trim()}
          className="shrink-0 rounded-lg bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 text-white px-3 py-2 text-xs transition">
          {disabled ? '…' : '↵'}
        </button>
      </div>
    </div>
...
export function ConversationPanel() {
  ...
  return (
    <>
      <button onClick={toggleOpen} aria-label="Toggle conversation panel"
        className="fixed bottom-16 right-4 z-50 w-10 h-10 rounded-full bg-indigo-600 hover:bg-indigo-500 shadow-lg flex items-center justify-center text-white text-base transition-transform hover:scale-105">
        {open ? '✕' : '💬'}
      </button>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-1">
              {messages.length === 0 && (
                <div className="text-center text-zinc-600 text-xs py-8">
                  <p className="mb-3">Query platform data or trigger admin actions.</p>
                  <div className="flex flex-col gap-1.5">
                    {['Platform revenue today', 'Which tenants have errors?', 'Agent fleet status'].map((s) => (
                      <button key={s} onClick={() => sendMessage(s)}
                        className="text-xs rounded-lg border border-surface-border bg-surface-overlay hover:bg-surface-overlay/80 text-zinc-400 px-3 py-1.5 transition text-left">{s}</button>
                    ))}
                  </div>
                </div>
              )}
