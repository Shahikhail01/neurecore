/**
 * Error Handling Utilities for NeureCore Frontend (Admin)
 *
...
export function logError(
  ...
  // Build a plain serializable object to avoid empty `{}` prints in some consoles
  const safeDetails = parsed.details
    ? (() => {
        try {
          return JSON.parse(JSON.stringify(parsed.details));
        } catch (e) {
