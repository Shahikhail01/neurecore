          ...
          >
            ...
            >
              <RecursionField
                ...
                onlyRenderProperties
                filterProperties={(s) =>
                  s["x-component"] !== "AssociationFilter"
                }
              />
            </div>
          </Col>
  ...
  return (
    <BlockContext.Provider value={blockValue}>
      <DataBlockProvider
        {...(props as any)}
        params={params}
        parentRecord={parentRecord || parentRecordFromHook}
      >
...
export const useFilterByTk = (blockProps?: any) => {
  ...
  if (!withoutTableFieldResource) {
    if (
      resource instanceof TableFieldResource ||
...
export const useParamsFromRecord = (props?: any) => {
  ...
  const collectionField = getCollectionJoinField(
    fieldSchema?.["x-decorator-props"]?.resource ||
      fieldSchema?.["x-decorator-props"]?.association,
  );
  ...
  if (
    record.__collection &&
    collectionField &&
    !["oho", "m2o", "obo"].includes(collectionField.interface)
  ) {
