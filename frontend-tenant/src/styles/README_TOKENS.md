...
export function Button({ children, variant = 'primary' }: ButtonProps) {
  const baseStyles = 'px-md py-sm rounded-md transition-colors duration-base font-medium';
  const variants = {
...
export function Alert({ type = 'info', title, message }: AlertProps) {
  ...
  return (
    <div className={`${style.bg} border-l-4 ${style.border} p-md rounded-md flex gap-md`}>
