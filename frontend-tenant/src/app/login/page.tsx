...
import { Eye, EyeOff, Loader2, LayoutDashboard, LogOut } from "lucide-react";
import { authService } from "@/services/auth.service";
import { useAuthStore } from "@/stores/authStore";

export default function LoginPage() {
  const router = useRouter();
  const { isAuthenticated, _hasHydrated, setUser, user, clearUser } = useAuthStore();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setUser(result.user);
      router.replace(result.user.tenantId ? "/dashboard" : "/onboarding");
    } catch (err: unknown) {
      if (mounted.current) {
        setError(
      ...
      <div className="w-full max-w-sm">
        ...
        {_hasHydrated && isAuthenticated && user ? (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 text-center space-y-4">
            ...
            <p className="text-xs text-zinc-500">You&apos;re already signed in</p>
            <Link
